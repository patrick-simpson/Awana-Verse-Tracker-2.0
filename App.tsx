...
const AdminPanel: React.FC<{ current: number; onUpdate: (n: number) => void; onClose: () => void }> = ({ current, onUpdate, onClose }) => {
  ...
  return (
    <div className="absolute bottom-20 right-0 w-80 glass-panel rounded-3xl p-6 text-white shadow-2xl">
      ...
      <div className="space-y-4">
        <input 
          ...
          className="w-full bg-black/40 border border-white/20 rounded-xl px-4 py-3 outline-none focus:border-yellow-400"
        />
        <button 
...
const App: React.FC = () => {
  ...
  return (
    <div ref={containerRef} className={`relative w-full h-screen transition-all duration-1000 bg-gradient-to-br ${theme.gradient} flex items-center justify-center overflow-hidden`}>
      ...
      {!isStarted && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-stone-900/95 backdrop-blur-xl text-center">
          <div className="p-12 glass-panel border-4 border-yellow-400 rounded-[3rem] max-w-lg shadow-2xl">
            <h1 className="text-5xl font-black text-white mb-4 uppercase tracking-tighter italic">Awana Africa</h1>
            ...
            <button
              ...
              className="bg-yellow-400 hover:bg-yellow-300 text-stone-900 font-black py-6 px-16 rounded-full text-2xl shadow-[0_12px_0_rgb(180,130,0)] hover:translate-y-2 transition-all active:scale-95 uppercase tracking-tighter"
            >
              Start Session
            </button>
          </div>
